"""
    ...
    See https://github.com/think-biq/LLV/blob/main/license.md for details.

    modified from: https://github.com/think-biq/LLV/blob/main/src/llv/gesicht.py
"""
...
"""
    Extended function to format MediaPipe data to LiveLink data 
    and send through UDP.
...
UDP_IP = config["UDP"].get("IP",fallback='127.0.0.1') # Doesn't matter
UDP_PORT = config["UDP"].getint("Port", fallback=11111)

frame = FaceFrame()
frame.VERSION = FaceFrame.VERSION # 6
frame.device_id = config["LiveLink"].get("device_id", fallback='WebCam') # Ignored
frame.subject_name = config["LiveLink"].get("subject_name", fallback='BasicRole') # Ignored
frame.frame_time = {"frame_number":0, "sub_frame":0, "numerator":0, "denominator":0}
frame.blendshape_count = 61
for shapeName in FaceFrame.FACE_BLENDSHAPE_NAMES:
    frame.blendshapes[shapeName] = 0.0

as_dict = {}

def LiveLinkUpdate(MediaPipe_BlendShapes, mp_transform):

    # Convert transformation matrix to Euler angles
    r = mp_transform[0][0:3, 0:3]
    r = Rotation.from_matrix(r)
    angles = r.as_euler("zyx",degrees=False)
    as_dict["headRoll"]  = -angles[0]
    as_dict["headYaw"]   = -angles[1]
    as_dict["headPitch"] = -angles[2] 

    for shape in MediaPipe_BlendShapes:
       as_dict[shape.category_name] = shape.score
    for shapeName in FaceFrame.FACE_BLENDSHAPE_NAMES:
       if shapeName in as_dict:
          frame.blendshapes[shapeName] =  as_dict[shapeName]
